...
import { OnPlaceCardMouseOver } from '../../types/handlers';
...
function PlaceCard({onMouseOver, ...offer }: PlaceCardProps): JSX.Element {
  const {id, price, isFavorite, type, previewImage, title, rating, isPremium} = offer;
  const url = AppRoute.Room.replace('id', `${id}`);

  return (
    <article className="cities__card place-card" onMouseOver={(): void => onMouseOver(offer)}>
